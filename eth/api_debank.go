...
func (api *DebankAPI) DebankBlock(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash) (*ptypes.DebankOutPut, error) {
	...
	if api.eth.APIBackend.ChainConfig().IsPrague(block.Number(), block.Time(), blockCtx.ArbOSVersion) || api.eth.APIBackend.ChainConfig().IsVerkle(block.Number(), block.Time()) {
		core.ProcessParentBlockHash(block.ParentHash(), evm)
	}
	var (
		txs     = block.Transactions()
		signer  = types.MakeSigner(api.eth.APIBackend.ChainConfig(), block.Number(), block.Time(), blockCtx.ArbOSVersion)
	...
	for i, tx := range txs {
		...
		receipt, _, err := core.ApplyTransactionWithEVM(msg, gp, statedb, block.Number(), block.Hash(), tx, usedGas, evm, nil)
		if err != nil {
			return nil, fmt.Errorf("could not apply tx %d [%v]: %w", i, tx.Hash().Hex(), err)
		}
